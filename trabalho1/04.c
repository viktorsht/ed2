...
tno *toLeft(tno *root){
    ...
    root->right = new->left;
    new->left = root;
    new->bf = getHeight(new->right, 0) - getHeight(new->left, 0);
    root->bf = getHeight(root->right, 0) - getHeight(root->left, 0);
    return new;
}
...
tno *insertTree(tno *root, tno *new, tno *result){
    if(root != NULL){
        result = root;
        // strcmp(new->data,root->data) == 0
        // new->data[0] < root->data[0]
        if(strcmp(new->data,root->data) < 0) root->left = insertTree(root->left, new, result);
        ...
        switch(root->bf){
            case 2:
                // ROTAÇAÕ SIMPLES
                if(root->right->bf >= 0) root = toLeft(root);
                else { // ROTAÇÃO DUPLA
            ...
            case -2:
                // ROTAÇAÕ SIMPLES
                if(root->left->bf <= 0) root = toRight(root);
                else { // ROTAÇÃO DUPLA
...
tno *search(tno *root, tdata x, tno *result){   
    ...
    // if(root != NULL){
    //     printf("%s == %s = %d\n", x, root->data, strcmp(x, root->data) == 0);
    // }
